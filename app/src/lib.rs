...
// Warp Lite is in the middle of deleting product/cloud/AI surfaces. Large
// compatibility tombstones intentionally leave unused imports, values, and
// unreachable match arms until the surrounding GUI code is fully collapsed.
#![allow(
    dead_code,
    non_snake_case,
...
pub use ai::agent::AIAgentActionResultType;
use ai::agent_conversations_model::AgentConversationsModel;
pub use ai::agent_management::AgentNotificationsModel;
...
fn initialize_app(
    ...
    ctx.add_singleton_model(|ctx| {
        ...
        let mut manager = crate::ai::api_keys::ApiKeyManager::new(&mut *ctx);
        #[cfg(not(target_family = "wasm"))]
    ...
    ctx.add_singleton_model(|ctx| BlocklistAIPermissions::new(ctx));
    ctx.add_singleton_model(|_| {
        ai::blocklist::orchestration_events::OrchestrationEventService::default()
    });

    ctx.add_singleton_model(|ctx| RepoOutlines::new(ctx));
    ...
    ctx.add_singleton_model(|ctx| AgentConversationsModel::new(ctx));
    ...
    ctx.add_singleton_model(|_| ActiveAgentViewsModel::default());
    ctx.add_singleton_model(|_| AgentNotificationsModel::default());
    ...
    ctx.add_singleton_model(|ctx| LLMPreferences::new(ctx));
        ...
        LaunchMode::CommandLine {
            ...
            cfg_if::cfg_if! {
                if #[cfg(target_family = "wasm")] {
                    ...
                    eprintln!("The Warp Lite build does not include agent command-line support: {command:?}");
                    std::process::exit(1);
                }
            }
        }
